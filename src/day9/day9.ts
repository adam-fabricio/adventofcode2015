...
function calculaDistancia(localAtual: string, naoVisitado: string[], distancia: number, locais: Map<string, Map<string, number>>, distancias: number[]): void{
  if (naoVisitado.length == 0) {
    distancias.push(distancia);
  } else {
    naoVisitado.forEach((proximaCidade) => {
      const subMap: Map<string, number> = locais.get(localAtual) as Map<string, number>;
      const distanciaProximaCidade: number = subMap.get(proximaCidade) as number;
      const distanciaAtualizada: number = distancia + distanciaProximaCidade;
      calculaDistancia(proximaCidade, naoVisitado.filter((cidade) => cidade !== proximaCidade), distanciaAtualizada, locais, distancias)
...
export function solver(input: string, part: number ) {
  ...
  cidades.forEach(cidade => {
    calculaDistancia(cidade, cidades.filter((thisCidade) => cidade != thisCidade), 0, locais, distancias);
  });
