...
function calcula(
  vars: string[],
  logica: string,
  resultado: Map<string, number>
): number {
  let var1 = Number(vars[0]);
  if (isNaN(var1)) var1 = resultado.get(vars[0]) as number;
  if (logica === "NOT") {
    return ~var1 & 0xffff;
  } else {
    let var2 = Number(vars[1]);
    if (isNaN(var2)) var2 = resultado.get(vars[1]) as number;
    switch (logica) {
    case "AND":
      return var1 & var2;
    case "OR":
...
export function solver(
  ...
  if (part === 2) resultado.set("b", bValue);
  let i = 0;
  ...
  while (variaveis.size > 0) {
    variaveis.forEach((submap, variavel) => {
      ...
      const possivelResolver = vars.every(
        (item) => !isNaN(Number(item)) || resultado.has(item)
      );
      if (possivelResolver) {
